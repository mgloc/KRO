...
class windows:
    ...
    def createWidgets(self):
        ...
        self.can_width = 1280
        self.can_height = 800
        self.taille_case = min(self.can_width, self.can_height) / max(
    ...
    def place_robot(self, coord: tuple = (0, 0), angle=0, is_carrying=False) -> None:
        ...
        corner_br_coord = self.convert_coord_to_pixels(
            (coord[0] + 1, coord[1] + 1)
        )  # br = bottom right
        middle = (
            (corner_tl_coord[0] + corner_br_coord[0]) / 2,
            (corner_tl_coord[1] + corner_br_coord[1]) / 2,
        )
        middle_plus_angle = self.convert_coord_to_pixels((-sin[angle], cos[angle]))
        middle_top = (
            middle[0] + 0.5 * middle_plus_angle[0],
            middle[1] + 0.5 * middle_plus_angle[1],
        )
...
class controller:
    ...
    def send_robot_three_points(self, robot, coord1, coord2):
        assert robot in robot_list
        if robot.is_available_path():
            chemin1, end_clock = m_star.pathfinder(
                robot.coord, coord1, self.graph, clock, return_end_clock=True
            )
            chemin2 = m_star.pathfinder(
                coord1, coord2, self.graph, end_clock, custom_clock=True
            )
    ...
    def send_robot_pick_up(self, robot, shelf, coord):
        ...
        if robot.is_available_path() and robot.is_available_pick_up():
            ...
            chemin_armoire, end_clock = m_star.pathfinder(
                robot.coord,
                shelf.coord,
                self.graph,
                clock,
                return_end_clock=True,
                robot_coord_list=reservation_list,
            )
            end_clock += database.pick_move_time
            chemin_coord = m_star.pathfinder(
                shelf.coord,
                coord,
                self.graph,
                end_clock,
                custom_clock=True,
                shelf=True,
                shelf_coord_list=[s.coord for s in shelf_list],
                robot_coord_list=reservation_list,
            )
